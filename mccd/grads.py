...
class CoeffLocGrad(GradParent, PowerMethod):
    ...
    def cost(self, x, y=None, verbose=False):
        r""" Compute data fidelity term. ``y`` is unused (it's just so ``modopt.opt.algorithms.Condat``
        ...
        cost_val = 0.5 * np.linalg.norm(
            self.obs_weights * (self._current_rec + self.FdH_glob - self.obs_data) * self.SNR_weights) ** 2
        return cost_val
...
class CoeffGlobGrad(GradParent, PowerMethod):
    ...
    def cost(self, x, y=None, verbose=False):
        ...
        if isinstance(self._current_rec, type(None)):
            self._current_rec = self.MX(x)
        cost_val = 0.5 * np.linalg.norm(
            self.obs_weights * (self._current_rec + self.FdH_loc - self.obs_data) * self.SNR_weights) ** 2
        return cost_val
...
class SourceLocGrad(GradParent, PowerMethod):
    ...
    def MX(self, transf_S):
        ...
        """
        S = utils.rca_format(
            np.array([filter_convolve(transf_Sj, self.filters, filter_rot=True) for transf_Sj in transf_S]))
    ...
    def cost(self, x, y=None, verbose=False):
        r""" Compute data fidelity term. ``y`` is unused (it's just so
