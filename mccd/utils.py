...
def acc_sig_maps(shap_im,ker_stack,sig_est,flux_est,flux_ref,upfact,w,sig_data=None):
    """ Applies acc_sig_map() several times.
...
def flux_estimate(im,cent=None,sigma=4): # [TL] New function
    """Estimate flux for one image (see SPRITE paper, section 3.4.1., subsection 'Photometric flux').
    """
    flux = 0
    rad = 3.*sigma
    if cent is None:
        CE = CentroidEstimator(im, sig=sigma) # [Jerome] here I put sig=15 only for the mask/nomask test (51*51 pictures ?)
        CE.estimate()
        cent = [CE.xc, CE.yc] # [max_coord[0] for max_coord in np.where(im==np.max(im))]
    shap = im.shape
    for i in range(0,shap[0]):
        for j in range(0,shap[1]):
...
def shift_ker_stack(shifts,upfact,lanc_rad=8): # [TL] New function
...
class GraphBuilder(object):
    ...
    def pick_emax(self, epsilon=1e-15):
        ...
        # MODIF [TL] TODO
        # Fixed e_max. It was too big before
        return 1.
...
def match_psfs(test_stars,PSFs):
    ...
    test_fluxes = flux_estimate_stack(test_stars, sigmas=sigmas)  # [TL] to modify

    # PSF calculation
    check_psf_moms = [gs.hsm.FindAdaptiveMom(gs.Image(star),
                guess_sig=psf_size, strict=False) for star in reg_format(PSFs)]
    check_psf_shapes = np.array([[moms.observed_shape.g1,
                             moms.observed_shape.g2,
                             moms.moments_sigma,
                             int(bool(moms.error_message))]
                        for moms in check_psf_moms])
    ...
    check_psf_test_fluxes = flux_estimate_stack(PSFs, sigmas=check_psf_sigmas)  # [TL] to modify
    ...
    shift_kernels, _ = shift_ker_stack(test_shifts,upfact,lanc_rad = lanc_rad) # [TL] simgas modif
    ...
    # (Changin the way the flux are defined for PSFEx) Instead of:
