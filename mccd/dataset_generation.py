...
class GenerateRealisticDataset(object):
    ...
    **Usage example**::
    ...
    def init_random_positions(self):
        ...
        self.mean_star_qt = int(
            np.ceil(
                np.random.uniform(
                    self.range_mean_star_qt[0], self.range_mean_star_qt[1]
                )
            )
        )
        ...
        for ccd_it in range(self.n_ccd):
            ...
            star_dev_nb = np.ceil(
                np.random.uniform(self.range_dev_star_nb[0], self.range_dev_star_nb[1])
            )
            # Calculate current CCD star number
            current_star_nb = int(self.mean_star_qt + star_dev_nb)

            # Simulate random positions
            x = np.random.uniform(0, self.loc2glob.x_npix, size=(current_star_nb))
            y = np.random.uniform(0, self.loc2glob.y_npix, size=(current_star_nb))
            ...
            if self.positions is not None:
                self.positions = np.concatenate((self.positions, current_pos), axis=0)
                self.ccd_list = np.concatenate((self.ccd_list, current_ccd), axis=0)
            else:
    ...
    def scale_fwhms(self, fwhms):
        ...
        scaled_fwhms = scaled_fwhms / (np.max(scaled_fwhms) - np.min(scaled_fwhms))
        # Multiply by the desired total range and restore the mean
        scaled_fwhms = (
            scaled_fwhms * 2 * self.max_fwhm_var + self.exposure_sim.mean_fwhm
        )
        return scaled_fwhms
    ...
    def generate_train_data(self):
        ...
        new_vignets = np.zeros((e1s.shape[0], self.image_size, self.image_size))
        ...
        for it in range(e1s.shape[0]):
            ...
            psf.drawImage(image=image_epsf, offset=rand_shift, scale=self.pix_scale)
        ...
        if self.save_realisation:
            ...
            save_str = self.output_path + "exposure_sim" + "-" + cat_id_str + ".npy"
    ...
    def generate_test_data(self, grid_pos_bool=False, x_grid=5, y_grid=10):
        ...
        for it in range(test_e1s.shape[0]):
            # PSF generation. Define size
            psf = gs.Moffat(fwhm=test_fwhms[it], beta=self.beta_psf)
            # Define the Flux
            psf = psf.withFlux(self.psf_flux)
            # Define the shear
...
class MomentInterpolator(object):
    ...
    def __init__(
        self, moment_map, n_neighbors=1000, rbf_function="thin_plate", loc2glob=None
    ):
        ...
        # Generate local generic grid
        x_lin = np.linspace(
            start=self.bin_x / 2, stop=self.x_pix - self.bin_x / 2, num=self.x_grid
        )
        y_lin = np.linspace(
            start=self.bin_y / 2, stop=self.y_pix - self.bin_y / 2, num=self.y_grid
        )
...
class AtmosphereGenerator(object):
    ...
    def power_fun(self, freq):
        ...
        # r = self.r_trunc
        return (freq**2 + 1 / (self.theta_zero**2)) ** (-11 / 6) * np.exp(
            -(freq**2) * (self.r_trunc**2)
        )
    ...
    def init_powerspectrum(self):
        ...
        self.my_ps = gs.PowerSpectrum(
            e_power_function=self.power_fun, b_power_function=self.power_fun
        )
    ...
    def plot_realisation(self, ccd_corner=None, save_path=None, save_fig=False):
        ...
        if ccd_corner is None:
            ccd_corner = int(np.floor(self.ngrid / 2.0))
        ...
        if save_fig:
            plt.savefig(save_path + "focal_plane_atmosphere.pdf", bbox_inches="tight")
        plt.show()
        ...
        plt.imshow(
            self.g1[
                ccd_corner : ccd_corner + p_ccd_x, ccd_corner : ccd_corner + p_ccd_y
            ].T
        )
        plt.colorbar()
        plt.gca().set_title("g1", fontsize="18")
    ...
    def plot_correlation(
        ...
        kmin = 2.0 * np.pi / (self.ngrid * self.grid_spacing) / kmin_factor
        ...
        if save_fig:
            plt.savefig(save_path + "correlation_atmosphere.pdf", bbox_inches="tight")
        plt.show()
...
class ExposureSimulation(object):
    ...
    def init_exposure(self):
        ...
        self.atmosphere = mccd.dataset_generation.AtmosphereGenerator(
            loc2glob=self.loc2glob, **self.atmos_kwargs
        )
        ...
        self.e1_interp = mccd.dataset_generation.MomentInterpolator(
            moment_map=self.e1_bins, loc2glob=self.loc2glob, **self.e1_kwargs
        )
        ...
        self.e2_interp = mccd.dataset_generation.MomentInterpolator(
            moment_map=self.e2_bins, loc2glob=self.loc2glob, **self.e2_kwargs
        )
        ...
        while (
            self.mean_fwhm < self.fwhm_range[0] or self.mean_fwhm > self.fwhm_range[1]
        ):
            self.mean_fwhm = self.fwhm_dist.ppf(np.random.rand(1))
